...
class LessonController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            $data = $request->validate([
                ...
                'teacher_id'        => 'nullable|integer|exists:users,id', // مهم! مش جدول teachers
                'timetable_period_id' => 'nullable|integer|exists:timetable_periods,id', // للحفاظ على التوافق مع from_period
                'timetable_period_ids' => 'nullable|array',
                'timetable_period_ids.*' => 'integer|exists:timetable_periods,id',
                'is_featured'       => 'boolean',
                'expiryPeriod'      => 'required|string',
            ...
            $lesson = Lesson::create([
                ...
                'expiry_period' => $data['expiryPeriod'],
    ...
    {
        $lesson = Lesson::with(['lectures.files', 'timetablePeriods.day', 'timetablePeriods.category'])->findOrFail($id);
        $teachers = User::where('user_type','teacher')->get();
        $categories = Category::with('children')->whereNull('parent_id')->get();
        $lectures = $lesson->lectures; // استخراج المحاضرات
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                // لو مفيش صورة جديدة، خليه على القديم

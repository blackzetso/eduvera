...
function confirmDelete(stream) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('admin.live-streams.destroy', stream.id), {
...
function getRemaining(stream) {
    ...
    // end_datetime from index is 'Y-m-d H:i' format — parse as local time
...
function startStream(stream) {
    // Navigate to the room page (controller handles status change)
    router.visit(route('admin.live-streams.room', stream.id))
}
...
function endStream(stream) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.patch(route('admin.live-streams.update-status', stream.id), { status: 'ended' }, {
        ...
        <div class="page-content-wrapper border">
            ...
            <div class="row mb-3 align-items-center">
                ...
                <div class="col-auto d-flex gap-2">
                    <Link :href="route('admin.settings.teams')" class="btn btn-outline-secondary btn-sm">
                        <i class="bi bi-gear me-1"></i> إعدادات Teams
                    </Link>
                    <Link :href="route('admin.live-streams.create')" class="btn btn-primary btn-sm">
                                    ...
                                    <td>
                                        <div class="d-flex gap-1 flex-wrap">
                                            ...
                                            <template v-else-if="stream.status === 'live'">
                                                <Link
                                                    :href="route('admin.live-streams.room', stream.id)"
                                            ...
                                            <!-- التفاصيل -->
                                            <Link
                                                :href="route('admin.live-streams.show', stream.id)"
                                            ...
                                            <Link
                                                ...
                                                :href="route('admin.live-streams.edit', stream.id)"

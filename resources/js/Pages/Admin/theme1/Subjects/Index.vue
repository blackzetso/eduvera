<script setup>
import { ref, watch } from 'vue'
import AppLayout from '@/Pages/Admin/theme1/Layout/App.vue'
import { Head, Link, router } from '@inertiajs/vue3'
import { route } from 'ziggy-js'
import Swal from 'sweetalert2'
import { toast } from 'vue3-toastify'

const props = defineProps({
  subjects: Object,
  filters: Object
})

// ✅ حذف مادة
function confirmDelete(id) {
  Swal.fire({
    title: 'هل أنت متأكد؟',
    text: "لن تتمكن من التراجع عن هذا الإجراء!",
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#d33',
    cancelButtonColor: '#3085d6',
    confirmButtonText: 'نعم، احذف',
    cancelButtonText: 'إلغاء'
  }).then((result) => {
    if (result.isConfirmed) {
      router.delete(route('admin.subjects.destroy', id), {
        onSuccess: () => {
          Swal.fire('تم الحذف!', 'تم حذف المادة بنجاح.', 'success')
        },
        onError: () => {
          Swal.fire('خطأ!', 'حدثت مشكلة أثناء الحذف.', 'error')
        }
      })
    }
  })
}

// ✅ البحث
const search = ref(props.filters?.search ?? '')
watch(search, (value) => {
  if (value) {
    router.get(route('admin.subjects.search', value), {}, {
      preserveState: true,
      replace: true,
    })
  } else {
    router.get(route('admin.subjects.index'), {}, {
      preserveState: true,
      replace: true,
    })
  }
})
</script>

<template>
  <Head title="المواد الدراسية" />
  <AppLayout>
    <div class="page-content-wrapper border">
      <!-- Title & Actions -->
      <div class="row mb-3">
        <div class="col-3">
          <h1 class="h3 mb-0">المواد الدراسية</h1>
        </div>
        <div class="col-7">
          <input
            class="form-control"
            v-model="search"
            name="search"
            placeholder="ابحث عن مادة..."
          />
        </div>
        <div class="col-2 text-center">
          <Link :href="route('admin.subjects.create')" class="btn btn-success-soft btn-round">
            <i class="bi bi-plus"></i>  
          </Link>
        </div>
      </div>

      <!-- Table -->
      <div class="card card-body bg-transparent pb-0 border mb-4">
        <div class="table-responsive border-0">
          <table class="table table-dark-gray align-middle p-4 mb-0 table-hover">
            <thead>
              <tr class="text-center">
                <th>#</th>
                <th>اسم المادة</th>
                <th>الصفوف الدراسية</th>
                <th>الإجراءات</th>
              </tr>
            </thead>

            <tbody>
              <tr
                v-for="(subject, index) in props.subjects.data"
                :key="subject.id"
                class="text-center"
              >
                <td>{{ index + 1 }}</td>
                <td>
                  <h6 class="mb-0">{{ subject.name }}</h6>
                </td>
                <td>
                  <div v-if="subject.categories && subject.categories.length > 0">
                    <span
                      v-for="(category, idx) in subject.categories"
                      :key="category.id"
                      class="badge bg-primary me-1"
                    >
                      {{ category.name }}
                      <span v-if="idx < subject.categories.length - 1">, </span>
                    </span>
                  </div>
                  <span v-else class="text-muted">غير معين</span>
                </td>
                <td>
                  <Link
                    :href="route('admin.subjects.edit', subject.id)"
                    class="btn btn-success-soft btn-round me-1"
                    title="تعديل"
                  >
                    <i class="bi bi-pencil-square"></i>
                  </Link>
                  <button
                    class="btn btn-danger-soft btn-round"
                    @click="confirmDelete(subject.id)"
                    title="حذف"
                  >
                    <i class="bi bi-trash"></i>
                  </button>
                </td>
              </tr>

              <!-- لما مفيش بيانات -->
              <tr v-if="!props.subjects.data.length" class="text-center">
                <td colspan="4" class="text-center py-4">
                  <i class="bi bi-inbox text-muted fs-4 d-block mb-2"></i>
                  <span class="text-muted">لا توجد مواد</span>
                </td>
              </tr>
            </tbody>
          </table>
        </div>

        <!-- Pagination -->
        <div class="card-footer bg-transparent pt-0">
          <div class="d-sm-flex justify-content-sm-between align-items-sm-center">
            <p class="mb-0 text-center text-sm-start">
              Showing {{ props.subjects.from }} to {{ props.subjects.to }} of
              {{ props.subjects.total }} entries
            </p>
            <nav class="d-flex justify-content-center mb-0" aria-label="navigation">
              <ul
                class="pagination pagination-sm pagination-primary-soft d-inline-block d-md-flex rounded mb-0"
              >
                <li
                  v-for="(link, key) in props.subjects.links"
                  :key="key"
                  class="page-item mb-0"
                  :class="{ active: link.active, disabled: !link.url }"
                >
                  <Link v-if="link.url" class="page-link" :href="link.url">
                    <template v-if="link.label.includes('Previous')">
                      <i class="bi bi-chevron-left"></i>
                    </template>
                    <template v-else-if="link.label.includes('Next')">
                      <i class="bi bi-chevron-right"></i>
                    </template>
                    <template v-else>
                      <span v-html="link.label"></span>
                    </template>
                  </Link>

                  <span v-else class="page-link">
                    <template v-if="link.label.includes('Previous')">
                      <i class="bi bi-chevron-left"></i>
                    </template>
                    <template v-else-if="link.label.includes('Next')">
                      <i class="bi bi-chevron-right"></i>
                    </template>
                    <template v-else>
                      <span v-html="link.label"></span>
                    </template>
                  </span>
                </li>
              </ul>
            </nav>
          </div>
        </div>
      </div>
    </div>
  </AppLayout>
</template>


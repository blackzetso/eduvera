...
function submitYoutubeUrl() {
    youtubeForm.patch(route('teacher.live-streams.video-url', props.stream.id), {
        onSuccess: () => {
...
function endStream() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            router.patch(route('teacher.live-streams.update-status', props.stream.id), { status: 'ended' }, {
        ...
        <div class="page-content-wrapper border">
            ...
            <div class="row mb-4 align-items-start">
                ...
                <div class="col-auto d-flex gap-2 flex-wrap justify-content-end">
                    <Link
                        :href="route('teacher.live-streams.index')"
                        class="btn btn-outline-secondary btn-sm"
                    >
                        <i class="bi bi-arrow-right me-1"></i>رجوع للبثوث
                    </Link>
                    <Link
                        v-if="stream.status === 'scheduled' || stream.status === 'live'"
                        :href="route('teacher.live-streams.room', stream.id)"
                        class="btn btn-sm"
                        :class="stream.status === 'live' ? 'btn-danger' : 'btn-success'"
                    >
                        <i class="bi me-1" :class="stream.status === 'live' ? 'bi-broadcast' : 'bi-play-fill'"></i>
                        {{ stream.status === 'live' ? 'ادخل الغرفة' : 'ابدأ البث' }}
                    </Link>
                    <button
                        ...
                        title="إنهاء البث"
                    >
                        <span v-if="updatingStatus" class="spinner-border spinner-border-sm"></span>
                ...
                <div v-if="stream.status === 'ended'" class="col-lg-8">
                    <div class="card shadow-sm mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <!-- No recording yet: offer YouTube URL form -->
                            <template v-else>
                                <p class="text-muted small mb-3">
                                    لا يوجد تسجيل بعد. إذا سجّلت الحصة على جهازك ورفعتها على YouTube، أضف الرابط هنا ليتمكن الطلاب من المشاهدة.
                                </p>
                                <div v-if="!showYoutubeForm">
                                    <button @click="showYoutubeForm = true" class="btn btn-sm btn-outline-danger">
                                        <i class="bi bi-youtube me-1"></i>إضافة رابط YouTube
                                    </button>
                                </div>
